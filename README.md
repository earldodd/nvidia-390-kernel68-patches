...
### Post-Fix Recurrence: Xorg configs regenerated by apt

After the system appeared fully working, a subsequent `apt upgrade` caused both the 6.8 and 5.15.0-179 kernels to fail at boot (blank screen / splash hang) again. Only 5.15.0-91 — the kernel without any DKMS nvidia module installed — booted correctly, because without `nvidia-drm` loaded there is no KMS device for the OutputClass to match.

**Root cause:** The `11-nvidia-prime.conf` file had been regenerated at `16:44` by a package postinst script that called the `gpu-manager` binary directly. The systemd mask had no effect on this direct invocation. `10-nvidia.conf` was still in its `.disabled` state from the original fix.

**Permanent fix:** Replace `11-nvidia-prime.conf` with a comment-only placeholder and apply `chattr +i` (immutable flag) to both files. The immutable flag prevents even root from overwriting the files, so postinst scripts cannot restore the OutputClass regardless of how they call `gpu-manager`.
...
**Step 1 — Mask `gpu-manager`** so it cannot regenerate `11-nvidia-prime.conf` at boot via systemd:
...
**Step 2 — Rename `10-nvidia.conf`** and lock it:

```bash
sudo mv /usr/share/X11/xorg.conf.d/10-nvidia.conf \
        /usr/share/X11/xorg.conf.d/10-nvidia.conf.disabled
sudo chattr +i /usr/share/X11/xorg.conf.d/10-nvidia.conf.disabled
```

**Step 3 — Replace `11-nvidia-prime.conf` with an immutable empty placeholder:**

```bash
# If the file exists, remove it first
sudo rm -f /usr/share/X11/xorg.conf.d/11-nvidia-prime.conf

# Create a comment-only placeholder (no OutputClass sections)
echo '# Intentionally empty — nvidia OutputClass disabled (nvidia_drv.so incompatible with Xorg 21.x)' | \
  sudo tee /usr/share/X11/xorg.conf.d/11-nvidia-prime.conf

# Lock it so package postinst scripts cannot overwrite it
sudo chattr +i /usr/share/X11/xorg.conf.d/11-nvidia-prime.conf
```

The `chattr +i` immutable flag is critical. Masking the systemd service alone is **not sufficient** — `apt` package postinst scripts call the `gpu-manager` binary directly, bypassing the systemd mask, and will silently regenerate `11-nvidia-prime.conf` on every nvidia package upgrade. With the immutable flag set, even root cannot overwrite the file without explicitly removing the flag first.
...
**Why this recurs:** `apt upgrade` of `nvidia-driver-390`, `nvidia-prime`, or `ubuntu-drivers-common` runs package postinst scripts that invoke the `gpu-manager` binary directly — not through systemd. The systemd mask only blocks the *service*; it does not prevent the binary from being called. As a result, `11-nvidia-prime.conf` gets silently regenerated after upgrades, restoring the OutputClass that loads `nvidia_drv.so` and causing a blank screen on the next boot.

**With `chattr +i` applied** (Step 3 above), the immutable flag blocks even root-level writes, so postinst scripts cannot overwrite the placeholder regardless of how they invoke `gpu-manager`.

After any nvidia package upgrade, verify the immutable flags are still set:

```bash
lsattr /usr/share/X11/xorg.conf.d/10-nvidia.conf.disabled
lsattr /usr/share/X11/xorg.conf.d/11-nvidia-prime.conf
# Both should show ----i---------e------- (the 'i' flag must be present)
```

If the `i` flag is missing on either file, re-apply it:
...
**To undo the lock** (e.g. when switching to a compatible driver in the future):
